...
export function advanceMatchday(session: GameSession) {
  ...
  // Advance calendar day + play available fixtures when possible
  try {
    const { advanceDay } = requireDay(w);
    advanceDay(w);
  } catch {
...
function requireDay(w: World) {
  // soft dependency
  return { advanceDay: (world: World) => {
    world.calendar.currentDate = bumpDate(world.calendar.currentDate);
  }};
}
...
export function listMarketPlayers(session: GameSession, opts?: { clubId?: string; limit?: number }) {
  ...
  const list = [...session.world.players.values()]
    .filter((p) => !p.retired)
...
export function getSquadWithValues(session: GameSession) {
  ...
  return club.squadPlayerIds
    .map((id) => snapshotPlayer(session.world, id))
    .filter(Boolean);
}
