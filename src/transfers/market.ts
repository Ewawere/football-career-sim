...
import { estimateMarketValue, askingPrice, yearsRemaining } from "../contracts/valuation.js";
...
export function sellerWillingness(
  ...
  const isSuperKid = player.age <= 20 && player.potential >= 90;
  const isFranchise = player.age <= 23 && player.potential >= 88 && player.ovr >= sellingClub.reputation - 5;
  ...
  if (isSuperKid) {
    ...
    } else if (offerFee >= ask * 1.25) {
      score += 10;
      reasons.push("big premium");
    }
  } else if (isEliteKid || isFranchise) {
    score -= 20;
    reasons.push("prize asset");
    if (offerFee >= ask * 1.35) {
      score += 18;
  ...
  if (sellingClub.reputation >= 88 && (isEliteKid || isSuperKid || isFranchise)) {
    score -= 12;
  ...
  score = Math.max(0, Math.min(100, score));
  return { interested: score >= 55, score, reasons };
}
